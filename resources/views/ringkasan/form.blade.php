...
<head>
  ...
  <!-- Script untuk menghitung total kata -->
  <script>
    function hitungTotalKata() {
      // Mengambil nilai teks dari textarea
      var teks = document.getElementById('teks').value;
      // Menghitung jumlah kata dalam teks
      var jumlahKata = teks.trim().split(/\s+/).length;

      // Menampilkan jumlah kata pada label total-kata
      document.getElementById('total-kata').innerText = jumlahKata + '/1000';
    }
  </script>
</head>
...
<body class="index-page sidebar-collapse">
  ...
  <div class="container centered-form">
    <div class="form-container">
      ...
      <form method="post" action="{{ route('ringkasan.proses') }}">
        @csrf
        <div class="form-group">
          <label for="teks">Masukkan teks yang ingin diringkas:</label>
          <textarea class="form-control" name="teks" id="teks" rows="5" placeholder="Ketik teks di sini" oninput="hitungTotalKata()"></textarea>
          <div class="total-kata" id="total-kata">0/1000</div>
          <!-- Menampilkan total kata -->
          @error('teks')
          <div class="error-message">{{ $message }}</div>
          @enderror
        </div>
        <button type="submit" class="btn btn-outline-success">Ringkas</button>
        <a href="/" class="btn btn-outline-danger" type="button">Kembali</a>
      </form>
    </div>
  </div>
        ...
        <div class="usage-section">
          <div class="usage-points">
            <ul style="text-align: left">
              <li>Masukkan teks yang ingin diringkas pada kotak yang tersedia.</li>
              <li>Klik tombol "Ringkas" untuk mendapatkan ringkasan teks.</li>
              <li>Jumlah kata teks yang Anda masukkan akan ditampilkan di bawah kotak teks.</li>
              <li>Pastikan teks yang dimasukkan tidak lebih dari 1000 kata.</li>
            </ul>
          </div>
          <div class="usage-download">
            <a href="e-book/CARA-PENGGUNAAN-FITUR-TEKS-TO-TEKS.pdf" download="Cara Penggunaan">Download e-book cara penggunaan (PDF)</a>
          </div>
        </div>
    </div>
  </div>
